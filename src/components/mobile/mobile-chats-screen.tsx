...
export function MobileChatsScreen({ initialChats }: { initialChats?: MobileConversationSummary[] | null }) {
  ...
  const eventChats = chats.filter((chat) => chat.kind === "event");
  const peopleChats = chats.filter((chat) => chat.kind !== "event");
      ...
      <div className="space-y-5">
        ...
        <section className="space-y-3">
          ...
          {visibleChats.map((chat) => (
            <Link
              key={chat.id}
              href={chat.eventSlug ? `/evento/${chat.eventSlug}` : `/chat/${chat.id}`}
