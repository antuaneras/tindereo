  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="rounded-full bg-white/90 px-4 py-3 text-sm font-semibold shadow-sm">Crear</div>
      </div>

      <div className="flex gap-2 rounded-full border border-[var(--line-soft)] bg-white/82 p-1 shadow-sm">
        {[
          { id: "media" as const, label: "Compartir" },
          { id: "event" as const, label: "Evento" }
        ].map((item) => (
          <button
            key={item.id}
            type="button"
            ...
            className={cn(
              ...
              activePanel === item.id ? "bg-[var(--text-main)] text-white" : "text-[var(--text-soft)]"
            )}
          >
            {item.label}
          </button>
      ...
      {activePanel === "media" ? (
        <MobileMediaCreator onOpenEvent={() => setActivePanel("event")} />
      ) : (
        <section className="rounded-[2rem] border border-[var(--line-soft)] bg-white/92 p-5 shadow-[0_18px_40px_rgba(29,22,15,0.06)]">
          ...
          <div className="mt-5 rounded-[1.6rem] border border-[var(--line-soft)] bg-[var(--bg-soft)] px-4 py-4 text-sm text-[var(--text-soft)]">
            <div className="flex items-center gap-2 font-semibold text-[var(--text-main)]">
              <Sparkles className="h-4 w-4" />
              Compartir y evento viven separados
            </div>
            <p className="mt-2 leading-6">
              Asi el flujo de subir historias o publicaciones se siente instantaneo, y crear evento solo aparece cuando
